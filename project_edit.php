...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tk_project SET project_name=%s, $project_code $project_text project_member=$project_member $project_start $project_end project_to_user=%s, project_status=%s WHERE id=%s",
...
if (trim($row_Recordset1['project_member'], ',') != '') {
	$row_Recordset1['project_member'] = explode(',', trim($row_Recordset1['project_member'], ','));
}

mysql_select_db($database_tankdb, $tankdb);
$query_Recordset2 = "SELECT * FROM tk_status_project ORDER BY task_status_pbackup1 ASC"; // 查询：项目所有状态
$Recordset2 = mysql_query($query_Recordset2, $tankdb) or die(mysql_error());
$row_Recordset2 = mysql_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysql_num_rows($Recordset2);

mysql_select_db($database_tankdb, $tankdb);
$query_Recordset4 = "SELECT * FROM tk_user WHERE tk_user_rank NOT LIKE '0' ORDER BY tk_display_name ASC"; // 查询：非禁用的所有用户
$query_Recordset4 = "SELECT * FROM tk_user WHERE tk_user_rank IN (3,4,5) ORDER BY tk_display_name ASC"; // 修改：只允许项目经理和普通用户为项目负责人
$Recordset4 = mysql_query($query_Recordset4, $tankdb) or die(mysql_error());
$row_Recordset4 = mysql_fetch_assoc($Recordset4);
$totalRows_Recordset4 = mysql_num_rows($Recordset4);

mysql_select_db($database_tankdb, $tankdb);
$query_Recordset5 = "SELECT * FROM tk_user WHERE tk_user_rank IN (2,3,4,5) ORDER BY tk_display_name ASC"; // 增加：允许项目经理、普通用户、访客为项目成员
...
<?php } ?>
          ...
          <tr>
            ...
            <td valign="top">
            	<ul class="ul-box">
            	<?php while ($user = mysql_fetch_assoc($Recordset5)) { ?>
            		<li>
            			<input type="checkbox" name="project_member[]" id="pm_<?php echo $user['uid']; ?>" value="<?php echo $user['uid']; ?>" <?php echo in_array($user['uid'], $row_Recordset1['project_member']) ? 'checked' : ''; ?>>
            			&nbsp;<label for="pm_<?php echo $user['uid']; ?>"><?php echo $user['tk_display_name']; ?></label>
            		</li>
            	<?php } ?>
            	</ul>
            </td>
