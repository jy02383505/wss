...
if (trim($row_DetailRS1['project_member'], ',') != '') {
	$pmemRS = mysql_query('select uid,tk_display_name from tk_user where uid in (' . trim($row_DetailRS1['project_member'], ',') . ') order by tk_display_name asc');
	$row_DetailRS1['project_members'] = array();
	while ($mem = mysql_fetch_assoc($pmemRS)) {
		$row_DetailRS1['project_members'][] = $mem;
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 ...
 require_once('tree.php'); ?></td>
      ...
      <td valign="top">
         <div style="overflow:auto; " id="main_right"><!-- right main --> 
          ...
          <tr>
            <td >
              <span>
                <b style="font-size:31.5px;line-height:40px;"><?php echo $row_DetailRS1['project_name']; ?></b>
                <a class="btn btn-sm btn-success" href="gantt_project.php?project_id=<?php echo $row_DetailRS1['id']; ?>&pagetab=<?php echo $_GET[pagetab]; ?>" style="float:right;position:relative;right:30px;">甘特图</a>
              </span>
            </td>
          </tr>
		  <tr>
  ...
  <tr>
    <td class="info_task_title"><?php echo $multilingual_project_member; ?></td>
  	<td colspan="5">
  		<?php
  			foreach ($row_DetailRS1['project_members'] as $mu) {
  			echo '<a style="display:inline-block;background:#e0e0e0;padding:0 6px;border-radius:2px;-webkit-border-radius:2px;-moz-border-radius:2px;" href="user_view.php?recordID='.$mu['uid'].'">' . $mu['tk_display_name'] . '</a>&nbsp;&nbsp;&nbsp;&nbsp;';
  			}
  		?>
  	</td>
  </tr>
		  ...
		  <tr>
            ...
            <td width="13%">
              <a onClick="addfolder()" class="mouse_hover"><i class="icon-folder-open"></i> <?php echo $multilingual_project_file_addfolder; ?></a>
            </td>
			<td width="12%">
			  <a href="file.php?projectID=<?php echo $row_DetailRS1[id]; ?>&folder=1&pfile=1&pagetab=allfile" class="mouse_hover"><i class="icon-folder-open"></i> 打开文件夹 </a>
      </td>
			 <td width="12%">
			 ...
			 <td width="12%">
			 <a onClick="addcomm();" class="mouse_over"><i class="icon-comment"></i> <?php echo $multilingual_default_addcom; ?></a>			 </td>
			 <?php } ?>
			 
			 <?php if($_SESSION['MM_rank'] > "4" || $_SESSION['MM_uid'] == $row_DetailRS1['project_to_user']) { ?>
			 <td width="10%">
