...
$where1 = $where2 = '';
$where = "tk_project.project_member LIKE %s";

if ( $pagetabs == 'mprj' ) {
	$where .= "AND tk_project.project_to_user = $prjtouser AND";
} elseif ( $pagetabs == "jprj" ) {
	$where .= "AND tk_project.project_to_user <> $prjtouser AND";
} else {
	$where .= ' AND';
}
$query_Recordset1 = sprintf("SELECT * FROM tk_project 
							...
							GetSQLValueString(($_SESSION['MM_rank'] == 5 && $pagetabs == "allprj") ? '%%' : "%," . $prjtouser . ",%", "text"),
...
// echo "<pre>";
// print_r("pagetabs:".$pagetabs."<br>");
// print_r("prjtouser:".$prjtouser."<br>");
...
<thead>
  <tr>
	...
	<th><font color="#555"><?php echo $multilingual_project_member; ?></font></th>
...
<?php $temp_arr = array(); ?>
  <?php do { ?>
    <tr>
      ...
      <td>
      	<?php
      		$mems = trim($row_Recordset1['project_member'], ',');
      		if (!empty($mems)) {
				mysql_select_db($database_tankdb, $tankdb);
      			$mem_Users = mysql_query('SELECT uid,tk_display_name FROM tk_user WHERE uid IN ('.$mems.') ORDER BY tk_display_name asc');
                $users = array();
      			while ( $mu = mysql_fetch_assoc($mem_Users) ) {
                    array_push($users, $mu);
    ...
    <!-- 甘特图入口 -->
    <td>
        <a href="gantt_project.php?project_id=<?php echo $row_Recordset1['id']; ?>&pagetab=<?php echo $pagetabs; ?>">查看甘特图</a>
    </td>
      ...
      <?php 
        array_push($temp_arr, $row_Recordset1);
       ?>
