...
mysql_select_db($database_tankdb, $tankdb);
$query_Recordset_prj = sprintf("SELECT * FROM tk_project inner join tk_status_project on tk_project.project_status=tk_status_project.psid WHERE project_to_user = %s AND project_member LIKE %s ORDER BY project_lastupdate DESC", GetSQLValueString($colname_Recordset_prj, "text"), GetSQLValueString("%," .$colname_Recordset_prj. ",%", "text"));
$query_limit_Recordset_prj = sprintf("%s LIMIT %d, %d", $query_Recordset_prj, $startRow_Recordset_prj, $maxRows_Recordset_prj);
//dump($query_limit_Recordset_prj);
...
// 查询：用户参与和负责的所有的项目
mysql_select_db($database_tankdb, $tankdb);
$queryLog_CurUserProjs = sprintf("SELECT id FROM tk_project WHERE project_member LIKE %s", GetSQLValueString("%,". $colname_Recordset_log .",%", "text"));
$CurUserProjsLogset1 = mysql_query($queryLog_CurUserProjs, $tankdb) or die(mysql_error());
$cupmids = array();
while ( $row_cupmid = mysql_fetch_assoc($CurUserProjsLogset1)) {
	array_push($cupmids, $row_cupmid['id']);
}
$where = 'AND tk_task_byday.csa_tb_backup3 IN (' . (!empty($cupmids) ? implode(',', $cupmids) : '0') . ')';
...
WHERE csa_tb_backup2 = %s $where AND csa_tb_year LIKE %s ORDER BY csa_tb_year DESC", 
...
//dump($query_limit_Recordset_log);
...
// 查询：用户参与和负责的所有的项目
mysql_select_db($database_tankdb, $tankdb);
$query_CurUserProjs = sprintf("SELECT id FROM tk_project WHERE project_member LIKE %s", GetSQLValueString("%,". $colname_Recordset_task .",%", "text"));
...
$query_Recordset_task = sprintf("SELECT *, 
							...
							FROM tk_task 
								...
								WHERE $where tk_task.csa_to_user = %s 
...
//dump($query_limit_Recordset_task);
