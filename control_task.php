...
// 查询：当期登录的用户参与和负责的所有的项目		
if ( $_SESSION['MM_rank'] != 5 ) {	
	mysql_select_db($database_tankdb, $tankdb);
	$query_CurUserProjs = sprintf("SELECT id FROM tk_project WHERE project_member LIKE %s", GetSQLValueString("%,". $_SESSION['MM_uid'].",%", "text"));
	$CurUserProjsset1 = mysql_query($query_CurUserProjs, $tankdb) or die(mysql_error());
	$cupmids = array();
	while ( $row_cupmid = mysql_fetch_assoc($CurUserProjsset1)) {
		array_push($cupmids, $row_cupmid['id']);
	}
	$where .= ' tk_task.csa_project in (' . (!empty($cupmids) ? implode(',', $cupmids) : '0') . ') AND';
}
...
mysql_select_db($database_tankdb, $tankdb);
// 查询：当期登录的用户只能看到参与和负责的项目中未完成的
if ( $_SESSION['MM_rank'] != 5 ) {
	$outwhere .= ' AND tk_task.csa_project in (' . (!empty($cupmids) ? implode(',', $cupmids) : '0') . ')';
}
...
mysql_select_db($database_tankdb, $tankdb);
$filter_proj_where = $_SESSION['MM_rank'] == 5 ? '' : "id IN (" . (!empty($cupmids) ? implode(',', $cupmids) : '0') . ") AND ";
$query_Recordset_project = "SELECT * FROM tk_project inner join tk_status_project on tk_project.project_status=tk_status_project.psid WHERE $filter_proj_where task_status NOT LIKE '%$multilingual_dd_status_prjfinish%' ORDER BY project_name ASC";
...
// 查询：当期登录的用户参与和负责的所有的项目，相关联的用户
$ser_User_where = '';
if ( $_SESSION['MM_rank'] != 5 ) {
	mysql_select_db($database_tankdb, $tankdb);
	$query_CurUserProjMems = sprintf("SELECT project_member FROM tk_project WHERE project_member LIKE %s", GetSQLValueString("%,". $_SESSION['MM_uid'].",%", "text"));
	$CurUserProjMems = mysql_query($query_CurUserProjMems, $tankdb) or die(mysql_error());
	$cupmids2 = array();
	while ( $row_cupmid2 = mysql_fetch_assoc($CurUserProjMems) ) {
		$cupmids2 = array_merge($cupmids2, explode(',', trim($row_cupmid2['project_member'], ',')));
	}
	$cupmids2 = array_unique($cupmids2);
	$ser_User_where .= 'uid IN (' . (!empty($cupmids2) ? implode(',', $cupmids2) : '0') . ')';
} else {
	$ser_User_where .= "tk_user_rank NOT LIKE '0'";
}

mysql_select_db($database_tankdb, $tankdb);
$query_Recordset2 = "SELECT * FROM tk_user WHERE $ser_User_where ORDER BY tk_display_name ASC";
