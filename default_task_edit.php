...
&nbsp;&nbsp;            <a href="#" onclick="eduser();">[<?php echo $multilingual_tasklog_changeuser; ?>]</a>
  ...
  <tr>
    ...
    <?php 
      $sql = "select tid, csa_text task_name, csa_remark4 from tk_task where csa_project=$project_id and tid!=$row_Recordset_task[TID] and csa_remark4!=$row_Recordset_task[TID]";
      $rows = Q($sql);
     ?>
	 ...
	 if($row_Recordset_countlog['count_log'] == "0"){  
      ...
      <!-- a to trigger modal -->
      <a href="#moveThisTo" data-toggle="modal" style="margin-right:90px;"><i class="icon-move"></i> 移动至</a>

      <!-- Modal -->
      <div id="moveThisTo" class="modal hide fade" tabindex="-1">
        ...
        <div class="modal-body">
          <form method="post" class="form-horizontal">
            <div class="control-group">
              <label class="control-label">请指定目标任务</label>
              <div class="controls">
                <select name="csa_remark4" class="span3">
                  <option value="-1">顶级任务</option>
                  <?php 
                    foreach($rows as $value){
                      echo "<option value='$value[tid]'>$value[task_name]</option>";
                    }
                   ?>
                </select>
              </div>
            ...
            <div style="float:right;position:relative;margin:0 15px 15px 0;">
      ...
      <?php 
        if($_POST[csa_remark4]){
          $csa_remark4 = $_POST[csa_remark4];
          $tid = $_POST[tid];
          $insertSql = "UPDATE tk_task SET csa_remark4=$csa_remark4 WHERE tid=$tid";
          if(Q($insertSql, 'exec')){
